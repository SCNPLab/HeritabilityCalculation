...
region_labels.hov_whole=readtable('F:\IPCAS_TWIN\CBF\info\Info\HOA_labels_cortexLabels.xlsx');
region_labels.hov_whole=region_labels.hov_whole(:,1);
region_labels.hov_whole=table2cell(region_labels.hov_whole);
% label BNA
...
%  LabelsSig=cell(1,length(herita_c_aft_ppm));
...
%     b=bar(combinedA_E);
%     b=bar(herita_c_aft_ppm,'FaceColor',[0,0,1]);
 b=bar(herita_a_aft_ppm,'FaceColor',[1,0,0]);
...
%     set(gca,'FontSize',12);
    set(gca,'FontSize',22);
% %     title(temp_name);
% % %      saveas(1,fullfile(path.output,['SigHerita_aftPPM_' temp_name '_3.png']),'png');
% %    saveas(1,fullfile(path.output,['SigSharedEnv_aftPPM_' temp_name '.png']),'png');
% %       close all;
...
%                 RefImageDir=fullfile(path.atlas,ROIset{1});
%               elseif wholeGroupSetOrd<=4 && wholeGroupSetOrd>2
%                 RefImageDir=fullfile(path.atlas,ROIset{2});
%               elseif wholeGroupSetOrd<=6 && wholeGroupSetOrd>4
%                 RefImageDir=fullfile(path.atlas,ROIset{3});
%                 elseif wholeGroupSetOrd<=8 && wholeGroupSetOrd>6
%                 RefImageDir=fullfile(path.atlas,ROIset{4});
%        end
% %              clear V 
...
% %              end
% %             V.fname=fullfile(path.output,[temp_name '_CBF_heritability_aftPPM.nii']);
% %             V.dt(1)=16;
